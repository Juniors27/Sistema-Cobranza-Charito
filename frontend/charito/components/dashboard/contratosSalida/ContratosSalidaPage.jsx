...
function TarjetaResumen({ titulo, valor, detalle, icono: Icono, tono = "sky" }) {
  ...
  return (
    <article
      className={`rounded-3xl border p-5 shadow-[0_16px_34px_rgba(15,23,42,0.06)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_22px_44px_rgba(15,23,42,0.12)] ${estilos[tono]}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-3 text-4xl font-semibold">{valor}</div>
          <div className="mt-2 text-sm opacity-80">{detalle}</div>
        </div>
...
export default function ContratosSalidaPage() {
  ...
  return (
    <div>
      ...
      <section className="overflow-hidden rounded-4xl border border-slate-200 bg-white shadow-[0_20px_48px_rgba(15,23,42,0.08)]">
        <div className="border-b border-slate-200 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.18),transparent_32%),linear-gradient(135deg,#f8fafc_0%,#ffffff_40%,#ecfeff_100%)] px-6 py-7">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="rounded-2xl border border-white/80 bg-slate-950 px-4 py-4 text-white shadow-sm">
      ...
      <section className="mt-4">
        <TarjetaResumen
          titulo="Saldo asociado"
          valor={formatearMonto(salida.resumen.saldoTotal)}
          detalle={`${salida.resumen.cobradores} cobradores con promesas registradas`}
          icono={ReceiptText}
          tono="slate"
        />
      </section>

      <section className="mt-6 rounded-[30px] border border-slate-200 bg-[linear-gradient(180deg,rgba(255,255,255,1)_0%,rgba(248,250,252,1)_100%)] p-6 shadow-[0_18px_38px_rgba(15,23,42,0.06)]">
        ...
        {salida.gruposPorCobrador.length === 0 ? (
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {salida.gruposPorCobrador.map((grupo) => (
              <article
                key={grupo.id}
                className="rounded-3xl border border-slate-200 bg-linear-to-br from-white via-slate-50 to-sky-50 p-5 shadow-[0_14px_32px_rgba(15,23,42,0.05)] transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(15,23,42,0.1)]"
              >
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">
                  Cobrador
                </div>
                <h3 className="mt-3 text-2xl font-semibold text-slate-900">{grupo.nombre}</h3>
                ...
                <div className="mt-5 grid grid-cols-3 gap-3 text-center text-sm">
                  <div className="rounded-2xl bg-sky-50 px-3 py-3 text-sky-800">
                    <div className="text-xl font-semibold">{grupo.total}</div>
                    <div>Total</div>
                  </div>
                  <div className="rounded-2xl bg-slate-100 px-3 py-3 text-slate-800">
                    <div className="text-xl font-semibold">{grupo.entregados}</div>
                    <div>Entregados</div>
                  </div>
                  <div className="rounded-2xl bg-emerald-50 px-3 py-3 text-emerald-800">
                    <div className="text-xl font-semibold">{grupo.yaPagaron}</div>
                    <div>Pagaron</div>
                  </div>
                  <div className="rounded-2xl bg-amber-50 px-3 py-3 text-amber-800 col-span-3">
                    <div className="text-xl font-semibold">{grupo.pendientes}</div>
                    <div>Pendientes</div>
                  </div>
                </div>
              </article>
      ...
      <section className="mt-6 rounded-[28px] border border-slate-200 bg-white p-6 shadow-[0_18px_38px_rgba(15,23,42,0.06)]">
        <div className="flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">
              Historial detallado
          ...
          <div className="text-sm text-slate-500">{salida.contratosSalida.length} registros</div>
        </div>
        ...
        {salida.contratosSalida.length === 0 ? (
          ...
          <div className="mt-6 overflow-x-auto rounded-3xl border border-slate-200 bg-white shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_12px_30px_rgba(14,165,233,0.08)]">
            <table className="w-full min-w-330 text-sm">
              ...
              <tbody>
                {salida.contratosSalida.map((contrato) => (
                  <tr
                    key={contrato.id}
